...
try {
  console.log("CSV Validator Worker loaded");
      ...
      for (let i = 0; i < parts.length; i++) {
        ...
        if (!currentProperties || !currentProperties[part]) {
          ...
          if (
            ...
            currentSchema.items.properties[part]
          ) {
    ...
    switch (targetType) {
      ...
      case "array":
        ...
        for (const item of items) {
          ...
          const itemPropSchema = itemSchemaType
            ? { type: itemSchemaType }
            : null;
          ...
          if (
            processedItem !== IGNORE_VALUE &&
            (processedItem !== null ||
              (item === "" && itemSchemaType === "string"))
          ) {
  ...
  function getValueFromPath(obj, path) {
    ...
    for (const part of parts) {
      ...
      const key = part.includes("~")
        ...
        : part; // Decode JSON Pointer encoding
      current = current[key];
    }
    return current;
  }
  ...
  self.onmessage = (event) => {
    ...
    if (type === "validate") {
      ...
      console.log(
        `Worker: Received validation request for ${csvData?.length} rows.`,
      );
      ...
      try {
          ...
          try {
            ...
            for (const key in originalRowData) {
              if (Object.hasOwnProperty.call(originalRowData, key)) {
                ...
                const convertedValue = convertValueBasedOnSchema(
                  rawValue,
                  propSchema,
                );
          ...
          if (!valid) {
            ...
            if (ajvErrors.length > 0) {
              ...
              const formattedAjvErrors = ajvErrors.map((error) => {
                ...
                const propertyName = error?.instancePath?.startsWith("/")
                  ? error.instancePath.substring(1).replace(/\//g, ".")
                  : error?.instancePath?.replace(/\//g, ".") ||
                    "Unknown Property"; // Convert path to dot notation

                // Attempt to get the problematic value
                const problematicValue = getValueFromPath(
                  processedRowData,
                  error.instancePath,
                );
                const valueString =
            ...
            } else if (!conversionErrorMsg) {
              // Generic if no other errors
              rowErrors.push({
                property: "Unknown",
                message: "Validation failed (reason unknown).",
              });
              totalErrorsCount++;
            }
          }
        ...
        console.log(
          `Worker: Validation complete. Total errors: ${totalErrorsCount}, Total warnings: ${totalWarningsCount}`,
        );
        self.postMessage({
          type: "complete",
          payload: {
  ...
  // Post error to main thread if import or top-level error occurs
  self.postMessage({
    type: "error",
    payload: {
      message:
