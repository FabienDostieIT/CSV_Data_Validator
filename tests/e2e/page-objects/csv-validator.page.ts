...
export class CSVValidatorPage {
  ...
  constructor(page: Page) {
    ...
    this.uploadSchemaButton = page.getByRole("button", {
      name: "Upload Schema",
    });
    this.showSchemaViewButton = page.getByRole("button", {
      name: /Show Schema View|Hide Schema View/,
    });
    this.schemaDocPanel = page
      .locator("h1", { hasText: "Schema Documentation" })
      .first()
      .locator('xpath=ancestor::div[contains(@class, "card")]');
    this.uploadCsvButton = page.getByRole("button", { name: "Upload CSV" });
    this.csvFileInput = page.locator(
      'input[type="file"][accept=".csv, text/csv"]',
    );
    this.schemaFileInput = page.locator(
  ...
  async getValidationStatus() {
    const successMessage = this.resultsPanel.getByText(
  ...
  async clickResultItem(rowNumber: number) {
    ...
    const item = this.resultItems
      .filter({ hasText: `Row ${rowNumber}` })
      .first();
    await item.click();
