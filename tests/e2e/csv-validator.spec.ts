...
| price | number | Price value (numeric only) | No |
  ...
  test("should populate and allow selection from schema dropdown", async () => {
    ...
    await expect(
      csvValidatorPage.schemaDropdownOptions.filter({ hasText: "event" }),
    ).toBeVisible();
    await expect(
      csvValidatorPage.schemaDropdownOptions.filter({ hasText: "place" }),
    ).toBeVisible();

    // Select a schema
    await csvValidatorPage.schemaDropdownOptions
